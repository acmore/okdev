...
- `--group <pod-a,pod-b>` targets an explicit pod group. Repeat `--group` to define multiple groups, and use either full pod names or the short names shown in `okdev status`.
- `--sequential` runs selected pods one-by-one when no groups are declared, or runs declared groups one group after another.
- `--parallel` runs declared groups in parallel. Within each group, pod execution still uses the normal fanout model.
...
- When `--log-dir` is used with multiple groups, logs are written into per-group subdirectories under the requested path.
