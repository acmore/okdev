...
- `--json`: capture each pod's result into a JSON envelope instead of streaming. A single resolved pod emits one object; multiple pods emit an array in selection order. Each envelope is `{"pod": "<name>", "exit": N, "stdout": "...", "stderr": "..."}`, with an `"error"` field added when the command could not be run or completed (in which case `exit` is `-1`).
