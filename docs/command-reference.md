...
- **Single-file downloads** auto-resume across transient stream failures. When the remote file can be stat'd, the sequential path pre-allocates a tmp file and streams via `tail -c +N | head -c L`, tracking the last-written offset so a retry picks up where it left off instead of restarting the whole transfer. This fixes the previous behavior where long-running downloads that tripped a mid-stream timeout would loop through the retry budget, inflating the progress counter past 100% and eventually failing without a file. `--parallel N > 1` opts single-file downloads into range-based chunking: the file is split into up to N disjoint byte ranges (at least ~16 MiB per chunk; smaller files fall back to sequential) and each range streams on its own exec. Aggregate throughput scales roughly linearly until `N ≈ 4` on high-RTT clusters, then diminishing returns and regressions set in. Single-file **uploads** still use the single-stream path (remote reassembly requires `dd oflag=seek_bytes` which isn't universally available on busybox).
...
- `--parallel N`: parallel streams per pod for directory copies *and* single-file downloads (default 1, max 16). For directory copies this splits the enumerated file list into roughly size-balanced tar buckets. For single-file downloads (≥ 16 MiB) it splits the file into up to N disjoint byte ranges read via `tail -c +N | head -c L` and assembled on disk via `WriteAt`; smaller files silently fall back to a single stream. Directory trees with fewer than 8 files also fall back to a single recursive tar. Single-file uploads ignore this flag. In multi-pod mode the effective per-pod parallelism is clamped so total streams (`fanout × parallel`) stays under a safe cap.
