...
- For Volcano pods annotated with `scheduling.volcano.sh/group-name` (or the legacy `scheduling.k8s.io/group-name`), `--details` also reads the associated `scheduling.volcano.sh/v1beta1` PodGroup in the session namespace. It shows the queue, phase, minimum membership, scheduler conditions and the latest 10 core Kubernetes events for that PodGroup's UID. Shared groups are reported once with their member pod names. JSON adds an optional `podGroups` array.
- PodGroup diagnostics are read-only and have a shared 5-second lookup budget. Missing CRDs/groups or denied access leave ordinary status usable and appear as `lookupError`; denied event access appears as `eventsError` alongside any available conditions. No associated annotation means no PodGroup lookup. Reading requires namespaced `get` on `podgroups.scheduling.volcano.sh` and `list` on core `events`; no CRD-management permissions are needed.
- PodGroup messages are scheduler-reported evidence, with their original reasons and timestamps. An explicit queue admission/quota message can explain a queue constraint, while a node-fit message points to placement constraints. A generic `Unschedulable` or `NotEnoughResources` message alone does not establish GPU quota exhaustion. Conditions and events may be historical; compare timestamps. See [Volcano PodGroup](https://volcano.sh/docs/concepts/podgroup/) for the resource model.
- `--details` is only valid when exactly one session is selected.
- Bare `status` uses the active session's saved config, context, and namespace, just like `status --session <name>`. Explicit `--config`, `--context`, and `--namespace` overrides remain authoritative; `status --all` uses the requested or discovered scope.
- Every status refresh that sees live pods caches a per-session snapshot locally. Snapshots are checked against the selected context, namespace, and run, and same-named sessions in another scope cannot overwrite them. A live workload suppresses the disappeared-session report; an API error is returned as an error, never treated as disappearance. When the session later **disappears without an `okdev down`** (TTL reclaim, quota eviction, admin delete), `okdev status` no longer stops at `No matching sessions found`: it prints the last known pod states (phase, reason, container terminations) plus any cluster events that outlived the objects (`Killing`, `Evicted`, `Preempted`, quota errors — events expire ~1h after emission). With `--output json` the same post-mortem is emitted as a structured object (`{"session", "context", "namespace", "found": false, "historical": true, "lastSeenAt", "workload", "pods", "events"}`) with capture time and scope explicitly labeling the snapshot as historical evidence, so monitoring loops can decide between "reclaimed, safe to recreate" and "evicted for resources, recreating won't help" without scraping text. `okdev up` and `okdev down` clear the snapshot — an intentional lifecycle transition is not a death cause.
...
- `--stdin` / `-i` streams local stdin to one foreground command without a TTY, buffering the whole input, or output prefixes: `printf 'hello\n' | okdev exec -i -- cat`. Binary and empty input are supported; EOF is forwarded. Without this flag, command execution does not forward stdin (interactive shells retain their existing behavior).
- Stdin mode uses the target pod or one explicit `--pod`. Fanout selectors, groups, JSON mode, `--script`, `--detach`, `--log-dir`, `--shell`, `--pkill`, and `--reset-gpu` are rejected before reading input. Redirect output locally when needed. `--timeout` and cancellation stop the local stream; they do not guarantee remote process termination. Stdin commands are never automatically replayed after a failure because input may already have been consumed. Remote exit codes are preserved; stream failures exit 69.
...
- Failure framing for `-- command...` fanout: when every failing pod merely reported a non-zero exit code from the command itself, the summary is headed `COMMAND EXITED NON-ZERO:` (followed by an explicit clarifier that the command was delivered and ran — the exit codes are the command's own results) and `okdev` exits with the remote status on a single-pod run (`exit 3` → `3`) or `1` on a multi-pod run — either way `okdev exec -- cmd && next` still short-circuits `&&` chains. When any pod could not run the command at all (unreachable, timeout, container gone), the summary is headed `FAILED:` and `okdev` exits `69`, letting scripts tell "my command failed" apart from "delivery failed" without parsing output.
...
- Readiness waiting reconnects after transient API transport failures and expired watches, with bounded backoff within the original `--wait-timeout`. Authentication, authorization, and invalid-request errors stop immediately.
- If readiness waiting times out or is interrupted, the submitted workload remains in the cluster. Setup has not run in that invocation, and no background watcher is started. The error includes commands to resume waiting and setup, inspect status, or explicitly cancel with `down --yes`, pinned to the same config, session, namespace, context, and owner. Increase the resume command's `--wait-timeout` if necessary; it does not replay `--reconcile` or `--reset-workspace`.
...
- When `sync.engine=syncthing`, `okdev up` refreshes the session's local Syncthing processes, starts background sync in bidirectional mode by default, then verifies the target and all intended primary-mapping mesh receivers before running hooks or reporting ready. A failed receiver causes `up` to fail with the workload retained for repair.
...
- Hook success and the `done` state mean the command exited zero; they do not independently verify setup effects. `up` shows each attempted hook's last 32 KiB of combined stdout/stderr on stderr, prefixed with the hook and pod name, including failures. Use explicit prerequisite/postcondition checks in the hook; see [lifecycle recipes](config-manifest.md#speclifecycle).
...
- The primary mapping also waits for every discovered mesh-eligible receiver (private-workspace pods other than the hub), including Pending pods. Each link must be connected and acknowledge the current hub/receiver indexes; equal file counts are insufficient. Pod replacement or a changing receiver set triggers verification again. The wait retains the initial receiver count, so a disappearing worker cannot turn failure into success. Pods not yet created by a controller are outside the discovered set.
- Additional mappings remain local-to-target only; they are not broadcast to workers. `sync status` reports `receivers` per mapping: scope, expected/connected/converged counts, Pod identities, pending names, and reasons. `--require-sync` and the final `up` sync gate use the same primary mesh verification. Mesh failures stop `up` before lifecycle hooks and the ready card; repair with `okdev sync` or rerun `up`.
- A shared workspace needs no mesh transfer when pods mount the same PVC/volume; inspect workload manifests and Pod mounts to confirm that sharing. Private `emptyDir` workspaces require a sidecar mesh. `status --details` labels path scope as configured topology and reports live receiver health separately; a configured topology is not evidence that files arrived.
