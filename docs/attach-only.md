...
`okdev status` and `okdev logs` are refused with an `attach-only` error before
cluster contact. Both report on a session okdev
...
check the config. This restriction does not apply to every CLI command:
`okdev list` still queries managed sessions and can contact the cluster; it does
not list the attach-only scope.
