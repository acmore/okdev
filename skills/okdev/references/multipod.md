...
**In-cluster addressing**: the same short names resolve **inside** the pods — okdev writes them into every pod's `/etc/hosts` at `up`/`restart` time, so multi-node launch scripts hardcode `MASTER_ADDR=master-0` (Ray, torchrun, etc.) instead of chasing pod IPs with `hostname -i`. Refresh lifecycle: okdev-driven recreations (`restart --pod`, `up --reconcile`) refresh immediately, and a **controller**-recreated pod (OOM, eviction, drain) is healed by the next `okdev exec`, which compares the recorded block against live pod UIDs and rewrites it before running anything; `okdev status` reports that drift without repairing it. Only the aliases self-heal — lifecycle hooks and interpod SSH keys on that pod still need an `okdev up`. PyTorchJob pods additionally get the training-operator's own per-replica Services and auto-injected `MASTER_ADDR`/`MASTER_PORT` env — for plain torch those env vars are already the canonical answer.
...
- The operator injects rendezvous env into **every** pod (`MASTER_ADDR`, `MASTER_PORT`, `RANK`, `WORLD_SIZE` and the five `PET_*` variables), with no `elasticPolicy` needed — a plain scaffolded job has all nine. That is the answer for distributed torch — and a trap for anything single-node started in the same pod (a Ray head, an inference server, a plain `python train.py`): it joins a rendezvous that never completes and **hangs with no error**, so there is no failure signal to react to, only elapsed time. Scrub the env for that one command — `okdev exec --pod master-0 -- env -u PET_MASTER_ADDR -u PET_MASTER_PORT -u PET_NNODES -u PET_NODE_RANK -u PET_NPROC_PER_NODE -u MASTER_ADDR -u MASTER_PORT -u RANK -u WORLD_SIZE <cmd>` (add `RAY_ADDRESS=127.0.0.1:6379` for a local Ray head). Never scrub it globally in a profile or lifecycle hook: that breaks the distributed launch just as silently. See `docs/troubleshooting.md`.
